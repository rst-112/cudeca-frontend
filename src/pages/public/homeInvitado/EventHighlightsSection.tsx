...
export const EventHighlightsSection = (): JSX.Element => {
  ...
  if (eventos.length === 0) {
    return (
      <section className="w-full bg-white dark:bg-slate-900 py-16 px-8">
        <div className="max-w-7xl mx-auto text-center">
          <h2 className="text-4xl font-bold text-slate-900 dark:text-white mb-4">Próximos Eventos</h2>
          <p className="text-slate-600 dark:text-slate-400">No hay eventos disponibles en este momento.</p>
        </div>
      </section>
    );
  }
                ...
                <div className="flex items-center justify-between pt-2 border-t border-slate-200 dark:border-slate-700">
                  {evento.objetivoRecaudacion && evento.objetivoRecaudacion > 0 ? (
                    <span className="font-bold text-slate-900 dark:text-white text-sm">
                      Meta: {new Intl.NumberFormat('es-ES', {
                        style: 'currency',
                        currency: 'EUR',
                    ...
                    <span className="text-slate-600 dark:text-slate-400 text-sm">Evento benéfico</span>
                  )}
                  <button
