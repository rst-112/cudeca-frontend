    ...
    >
      ...
      <div className="flex-1 flex flex-col gap-6 p-8 overflow-y-auto">
        {navigationItems.map((item) => (
          <div key={item.id} className="flex flex-col gap-2">
            <h3 className="text-base font-semibold text-[#00a651] dark:text-[#00d66a] flex items-center gap-2">
              {item.icon}
              {item.title}
            </h3>
              ...
              {item.subItems.map((subItem, index) => (
                <li key={index}>
                  {subItem.link ? (
                    ...
                    <a
                      href="#"
                      className="text-sm text-slate-600 dark:text-slate-400 hover:text-[#00a651] dark:hover:text-[#00d66a] transition-colors"
                    >
                      {subItem.label}
                    </a>
                  )}
      ...
      <footer className="border-t border-slate-200 dark:border-slate-700 p-8">
        <button
          ...
          className="w-full px-4 py-3 flex gap-2 items-center justify-center rounded-lg border-2 border-[#00a651] dark:border-[#00d66a] text-[#00a651] dark:text-[#00d66a] hover:bg-[#00a651] hover:bg-opacity-10 dark:hover:bg-[#00d66a] dark:hover:bg-opacity-10 transition-colors font-bold"
          type="button"
